      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid gap-5 lg:grid-cols-[360px_1fr]">
          ...
          <div className="space-y-5">{selected ? <>
            ...
            {selected.missions.length > 0 && <Card className="border-white/10 bg-slate-950/80 text-slate-100"><CardHeader><CardTitle>Misiones · ciclo independiente</CardTitle><CardDescription className="text-amber-300">Aprobar valida el análisis; RUNNING y DONE son inalcanzables y nunca ejecutan acciones.</CardDescription></CardHeader><CardContent className="space-y-3">{selected.missions.map((mission) => <div key={mission.id} className="rounded-xl border border-white/10 p-4"><div className="flex justify-between gap-3"><p className="font-bold">{mission.title}</p><Badge variant="outline">{mission.status}</Badge></div><p className="mt-2 text-sm text-slate-400">{mission.expectedOutput}</p><p className="mt-1 text-xs text-slate-600">{mission.rationale}</p><p className="mt-2 text-xs text-violet-300">Creada por {selected.agentId} · reporta a {selected.agentId === 'systems-manager-ai-v1' ? 'general-manager-ai-v3' : 'Diego'}</p><div className="mt-3 flex flex-wrap gap-2"><Button size="sm" onClick={() => decideMission(mission.id, 'APPROVE')}>Aprobar análisis</Button><Button size="sm" variant="outline" onClick={() => decideMission(mission.id, 'REQUEST_REVIEW')}>Revisar</Button><Button size="sm" variant="outline" onClick={() => decideMission(mission.id, 'REJECT')}>Rechazar</Button><Button size="sm" variant="outline" onClick={() => void captureMissionDecision(mission, 'EDIT')}>Editar</Button><Button size="sm" variant="outline" onClick={() => void captureMissionDecision(mission, 'POSTPONE')}>Posponer</Button><Button size="sm" variant="outline" onClick={() => void captureMissionDecision(mission, 'MARK_INCORRECT')}>Información incorrecta</Button><Button size="sm" variant="outline" onClick={() => decideMission(mission.id, 'BLOCK')}>Bloquear</Button></div></div>)}</CardContent></Card>}
