...
type Snapshot = { objectives: ContinuousObjectiveDisplay[]; allowedProjects: string[]; externalSources: typeof CONTINUOUS_OBJECTIVE_EXTERNAL_SOURCES[number][] };
...
export function parseContinuousObjectivesSnapshot(raw: unknown): Snapshot {
  ...
  if (externalSources.some((source) => !source || typeof source !== 'object'
    || !CONTINUOUS_OBJECTIVE_EXTERNAL_SOURCES.some((known) => known.id === (source as { id?: unknown }).id
      && known.status === (source as { status?: unknown }).status
      && known.label === (source as { label?: unknown }).label
      && known.note === (source as { note?: unknown }).note))) {
...
export function ContinuousObjectiveCard({ objective, disabled = false, pausing = false, onPause, onConfirmPause, onCancelPause, onResume }: {
  ...
  return (
    <article className="rounded-2xl border border-slate-800 bg-slate-950/80 p-5" aria-label={objective.title}>
      ...
      <details className="mt-4"><summary className="cursor-pointer text-sm font-medium text-slate-200">Criterios y fuentes autorizadas</summary><ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-slate-300">{objective.criteria.map((criterion, index) => <li key={index}>{criterion}</li>)}</ul><p className="mt-3 text-xs text-slate-400">Proyectos Codex: {objective.projectAllowlist.length ? objective.projectAllowlist.join(' · ') : 'Ninguno'}</p><p className="mt-2 text-xs text-slate-400">Fuentes externas: {objective.externalSources.length ? objective.externalSources.map((source) => externalSourceNames[source] ?? source).join(' · ') : 'Ninguna'}</p>{objective.externalSources.length > 0 && <p className="mt-2 text-xs text-amber-300">Las fuentes externas quedan bloqueadas y auditadas hasta instalar un conector independiente de sólo lectura en el runtime.</p>}</details>
    ...
    <div className="space-y-5">
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[360px_minmax(0,1fr)]">
        <form onSubmit={(event)=>void create(event)} className="space-y-4 rounded-2xl border border-cyan-400/20 bg-slate-950 p-5">
          ...
          <fieldset className="space-y-2"><legend className="mb-2 text-sm">Fuentes externas de sólo lectura (opcional)</legend>{(snapshot?.externalSources ?? CONTINUOUS_OBJECTIVE_EXTERNAL_SOURCES).map((source)=><label key={source.id} className="flex items-start gap-2 text-sm text-slate-300"><input type="checkbox" checked={externalSources.includes(source.id)} onChange={(event)=>setExternalSources((selected)=>event.target.checked?[...selected,source.id]:selected.filter((value)=>value!==source.id))} className="mt-1 accent-cyan-400" /><span>{source.label}<span className="block text-xs text-amber-300">{source.status === 'BLOCKED_REQUIRES_READONLY_BRIDGE' ? 'Requiere puente read-only' : 'Requiere conector del runtime'}</span></span></label>)}</fieldset>
