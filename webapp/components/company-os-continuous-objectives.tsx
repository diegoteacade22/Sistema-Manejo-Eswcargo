...
import type { ContinuousObjectiveView } from '@/lib/company-os/continuous-objective-types';

export type ContinuousObjectiveDisplay = ContinuousObjectiveView;
type Snapshot = { objectives: ContinuousObjectiveDisplay[]; allowedProjects: string[] };
...
export function ContinuousObjectiveCard({ objective, disabled = false, pausing = false, onPause, onConfirmPause, onCancelPause, onResume }: {
  ...
  return (
    <article className="rounded-2xl border border-slate-800 bg-slate-950/80 p-5" aria-label={objective.title}>
      ...
      <details className="mt-4"><summary className="cursor-pointer text-sm font-medium text-slate-200">Criterios y proyectos autorizados</summary><ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-slate-300">{objective.criteria.map((criterion, index) => <li key={index}>{criterion}</li>)}</ul><p className="mt-3 text-xs text-slate-400">Proyectos: {objective.projectAllowlist.join(' · ')}</p></details>
...
export function CompanyOsContinuousObjectives() {
  ...
  async function create(event: FormEvent) {
    ...
    try {
      await mutate({action:'CREATE',title:title.trim(),objective:objective.trim(),durationDays,projectAllowlist:projects,criteria:criteria.split('\n').map((item)=>item.trim()).filter(Boolean)});
      setTitle(''); setObjective(''); setCriteria(''); setProjects([]); setPauseTarget(null);
      setNotice('Objetivo creado y activo. El próximo barrido buscará trabajo en los proyectos elegidos.');
    ...
    <div className="space-y-5">
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[360px_minmax(0,1fr)]">
        <form onSubmit={(event)=>void create(event)} className="space-y-4 rounded-2xl border border-cyan-400/20 bg-slate-950 p-5">
          ...
          <fieldset className="space-y-2"><legend className="mb-2 text-sm">Proyectos permitidos</legend>{snapshot?.allowedProjects.map((project)=><label key={project} className="flex items-start gap-2 text-sm text-slate-300"><input type="checkbox" checked={projects.includes(project)} onChange={(event)=>setProjects((selected)=>event.target.checked?[...selected,project]:selected.filter((value)=>value!==project))} className="mt-1 accent-cyan-400" />{project}</label>)}{!snapshot ? <p className="text-xs text-slate-400">Cargando proyectos del servidor…</p> : snapshot.allowedProjects.length === 0 ? <p className="text-xs text-amber-300">No hay proyectos permitidos configurados.</p> : null}</fieldset>
          ...
          <Button type="submit" disabled={!!busy || !current || projects.length===0} className="w-full bg-cyan-400 text-slate-950 hover:bg-cyan-300">{busy==='CREATE'?<Loader2 className="mr-2 h-4 w-4 animate-spin" />:<Play className="mr-2 h-4 w-4" />}Crear y activar</Button>
        </form>
