...
async function savePaymentProof(file: File | null) {
...
export async function registerPaymentFromForm(formData: FormData) {
    ...
    try {
        ...
        const proofFileValue = formData.get('proof');
        ...
        const proofUrl = await savePaymentProof(proofFile);
        const fullDescription = proofUrl
            ? `${description || 'Pago a cuenta'} | Comprobante: ${proofUrl}`
        ...
        const transaction = await createPaymentLedgerEntry(prisma, {
            ...
            reference: reference || proofUrl || null,
        });
        ...
        revalidatePath('/collections');
        revalidatePath('/analytics/financial');
        return { success: true, transaction, proofUrl };
    } catch (error: unknown) {
