...
import { getInvPdfFileName, getPackingListPdfFileName, savePdfToDriveFolder } from '@/lib/document-storage';
...
async function buildPackingListDocument(shipmentId: number): Promise<PackingListDocument> {
    ...
    shipmentItems.forEach(item => {
        ...
        itemsHtml += `
                <tr>
                    ...
                    <td style="padding: 10px; border-bottom: 1px solid #eee; text-align: center; font-weight: bold; color: #F4AB3D;">${(item.order?.order_number && item.order.order_number < 900000) ? '#' + item.order.order_number : '-'}</td>
                </tr>
             `;
    });
    ...
    const fileName = getPackingListPdfFileName(shipment.invoice, shipment.shipment_number || shipment.id);
