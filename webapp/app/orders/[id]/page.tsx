...
async function getOrderDetails(id: string, user: AppUser) {
...
export default async function OrderPage(props: Props) {
    ...
    const order = await getOrderDetails(params.id, sessionUser);
    ...
    const effectiveShipment = order.shipment || order.items.find((item) => item.shipment)?.shipment;
                            ...
                            <div className="mt-1">
                                {isAdmin ? (
                                    <OrderStatusDialog
                                        ...
                                        currentShipmentId={effectiveShipment?.id ?? null}
                                        shipments={shipments}
                                    />
                                ) : (
