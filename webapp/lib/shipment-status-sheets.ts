...
export function buildShipmentSheetPlan(
  ...
  for (const candidate of candidates) {
    ...
    if (!valuesEqual(previousValue, canonicalStatus)) {
      updates.push({ range, previousValue, nextValue: canonicalStatus, shipmentNumber: candidate.shipmentNumber, sheet: 'CABE_ENVIOS', row });
    }
  }
  ...
  for (let index = 0; index < detailRowCount; index += 1) {
    ...
    if (!valuesEqual(previousValue, canonicalStatus)) {
      updates.push({ range, previousValue, nextValue: canonicalStatus, shipmentNumber, sheet: 'DETA_VENTAS', row });
    }
  }
...
export async function applyShipmentSheetPlan(plan: ShipmentSheetPlan) {
  ...
  if (returned.length !== plan.updates.length) throw new Error('No se pudieron verificar todas las celdas modificadas.');
...
export async function rollbackShipmentSheetPlan(plan: ShipmentSheetPlan) {
  ...
  const valuesToRestore = safeToRestore.filter((update) => update.previousValue !== null && update.previousValue !== undefined && String(update.previousValue) !== '');
  const rangesToClear = safeToRestore.filter((update) => update.previousValue === null || update.previousValue === undefined || String(update.previousValue) === '');
