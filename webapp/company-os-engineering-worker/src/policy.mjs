...
export function missionHash(mission) {
  const { expectedStateVersion: _mutableStateVersion, ...immutableMission } = mission;
  return createHash('sha256').update(canonicalJson({ contractVersion: CONTRACT_VERSION, ...immutableMission })).digest('hex');
}
...
export function validateClaim(claim, config, now = new Date()) {
  ...
  return { missionPaths, leasePaths };
}
