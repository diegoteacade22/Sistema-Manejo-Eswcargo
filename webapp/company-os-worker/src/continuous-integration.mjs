...
export function isSpecialistIntegrationClaim(claim) {
  return claim?.agentId === GENERAL && claim?.runtimePhase === SPECIALIST_INTEGRATION_PHASE;
}

export function continuousIntegrationResults(claim) {
  if (!isSpecialistIntegrationClaim(claim)) return [];
...
export function integrationContext(claim) {
  if (!isSpecialistIntegrationClaim(claim)) return claim.contextMessages || [];
